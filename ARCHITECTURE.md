...
- The frontend is served directly from the S3 website endpoint.
- The backend runs as a Docker container on EC2 instances launched by an Auto Scaling Group in private subnets.
- An internet-facing ALB routes traffic to the backend target group and uses `/ping` for instance health checks.
...
- The frontend CI pipeline uploads the static build to S3 so the website endpoint serves the latest bundle.
